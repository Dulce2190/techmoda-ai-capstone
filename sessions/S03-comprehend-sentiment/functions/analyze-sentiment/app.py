...
def _redact_pii(text, lang):
    """S12 - Detecta y anonimiza PII (email, telefono, nombre, etc.) antes de
    analizar/guardar una resena de cliente. Segunda capa de privacidad, distinta
    del guardrail de Bedrock (S9): esto protege datos que EL CLIENTE escribio,
    no la salida de un modelo generativo."""
    try:
        resp = comprehend.detect_pii_entities(Text=text, LanguageCode=lang)
    except Exception as e:
        print("DetectPiiEntities fallo (no bloqueante):", repr(e))
        return text, []
    ...
    for ent in sorted(entities, key=lambda e: e["BeginOffset"], reverse=True):
        start, end, kind = ent["BeginOffset"], ent["EndOffset"], ent["Type"]
        redacted = redacted[:start] + f"[{kind}]" + redacted[end:]

    found_types = sorted({e["Type"] for e in entities})
    return redacted, found_types


def _analyze_one(text):
    lang = _detect_language(text)
    clean_text, pii_found = _redact_pii(text, lang)
    s = comprehend.detect_sentiment(Text=clean_text, LanguageCode=lang)
    return {
        "text": clean_text,
        "originalHadPii": bool(pii_found),
        "piiTypesRedacted": pii_found,
        "language": lang,
